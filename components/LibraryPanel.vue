...
const draft = reactive({
  id: null, name: '', template: '', setup: '', style: '', styleLang: 'css',
})
...
function editComponent(c) {
  Object.assign(draft, { id: c.id, name: c.name, template: c.template, setup: c.setup, style: c.style, styleLang: c.styleLang })
  errorMsg.value = ''
  showEditor.value = true
}
...
async function save() {
  ...
  try {
    ...
    if (draft.styleLang === 'scss' && draft.style.trim()) {
      const res = await $fetch('/api/compile-scss', { method: 'POST', body: { source: draft.style } })
      compiledCss = res.css
    }
    saveComponent({ ...draft, compiledCss })
    showEditor.value = false
  } catch (err) {
    errorMsg.value = err.data?.error || err.message || 'SCSS compile failed'
  } finally {
    saving.value = false
  }
}
...
<template>
  <div class="library-panel">
    <p class="lib-hint">
      Your components: paste a Vue template, optional logic, and CSS or SCSS.
      PrimeFlex utilities and PrimeVue components work inside templates.
    </p>
    ...
    <Dialog v-model:visible="showEditor" :header="draft.id ? 'Edit component' : 'New component'" modal :style="{ width: '560px' }">
      ...
      <div class="ed-field">
        <label class="ed-label">Template (Vue)</label>
        <textarea class="ed-code" rows="7" v-model="draft.template" spellcheck="false" />
      </div>
      <div class="ed-field">
      ...
      <div class="ed-field">
        ...
        <textarea class="ed-code" rows="6" v-model="draft.style" spellcheck="false" />
      </div>
